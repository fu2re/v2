...
ENVIRONMENT: dict[str, list[str]] = {
    ...
    # Золото легендарных. Единственная рампа мира на грани потолка
    # насыщенности, и это допустимо: все игровые цвета холодные, а золото
    # тёплое — до ближайшего из них 0.216 при пороге 0.15. Верхняя ступень
...
def quantization_palette(gold: bool = False, beet: bool = False) -> np.ndarray:
    ...
    Золото по умолчанию исключено. Проверено на первом прогоне с ним: как
    только рампа появилась в наборе, к ней потянулись все тёплые светлые
    пиксели подряд — подсолнух и мотылёк пожелтели целиком. Золото обязано
