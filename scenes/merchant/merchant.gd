...
## Магазин усадьбы: торговец за СЕРЕБРО (GDD §8.2.2).
##
## Здесь запасаются перед выходом в лес — семенами и снаряжением.
...
## Номер витрины в прошлый раз. Список пересобирается только при смене
## витрины: полная пересборка каждый кадр уничтожала бы кнопку между
## нажатием и отпусканием, и покупка не срабатывала бы (CLAUDE.md).
