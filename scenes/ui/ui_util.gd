...
## Иконки валют. Монета читается быстрее слова и не зависит от языка —
## ребёнок семи лет узнаёт её раньше, чем дочитает «Серебро».
const COIN_SILVER := "res://art/currency/coin_silver.png"
const COIN_GOLD := "res://art/currency/coin_gold.png"
## Высота монеты в строке. Чуть больше кегля, чтобы монета не терялась
## между цифрами, но не разгоняла межстрочный интервал.
const COIN_SIZE := 44


## Строка кошелька с монетами вместо слов: «🪙 340   🥇 120».
##
## Собрана в ОДНОМ месте намеренно: кошелёк показан на пяти экранах, и пока
## каждый склеивал строку сам, они разъезжались по формату («Серебро %d»
## против «Серебро: %d») и по составу. Возвращает BBCode — вешать только
## на `RichTextLabel` с `bbcode_enabled`.
##
## Отрицательное значение означает «эту валюту здесь не показываем»:
## у торговца платят только серебром, в магазине нарядов — только золотом,
## и пустая графа сбивала бы с толку. Контуры валют не смешиваются
## намеренно (GDD §12), поэтому и в кошельке они разделимы.
##
## Строка приходит уже отцентрованной: `RichTextLabel` не знает
## `horizontal_alignment`, выравнивание у него — тег, и забыть его легко.
static func purse_bbcode(silver: int, gold: int = -1, extra: String = "") -> String:
	var parts: Array[String] = []
	if silver >= 0:
		parts.append("[img=%d]%s[/img] %d" % [COIN_SIZE, COIN_SILVER, silver])
	if gold >= 0:
		parts.append("[img=%d]%s[/img] %d" % [COIN_SIZE, COIN_GOLD, gold])
...
##
## Каждое место усадьбы (огород, коллекция, инвентарь) рисуется своим фоном,
