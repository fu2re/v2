...
## Сколько мелодий обязано быть в пуле каждого типа поляны — НЕ МЕНЬШЕ.
##
## Именно нижняя граница: добавить треков можно и нужно, а вот похудевший
## пул — тихая потеря контента. Равенство пулов между типами проверяется
## отдельно: десять мелодий у боя и одна у костра означают, что генерация
## оборвалась на середине.
const GLADE_POOL_MIN := 5
...
func _test_every_glade_type_has_a_pool() -> void:
	...
	for type: int in Glade.TYPE_KEYS:
		...
		check(count >= GLADE_POOL_MIN,
			"пул поляны «%s»: %d мелодий, меньше %d" % [key, count, GLADE_POOL_MIN])
		check(Jukebox.play_glade(key), "мелодия поляны «%s» не загрузилась" % key)
		sizes[key] = count

	# Размеры печатаются справкой, а не проверяются на равенство: у боя сейчас
	# вдвое больше мелодий, и это не поломка — боевых полян в ленте больше
	# всех, разнообразие там нужнее. Но перекос должен быть виден,
	# чтобы «у костра всего одна» не проехало незамеченным
	var report: Array[String] = []
	for key: String in sizes:
		report.append("%s %d" % [key, int(sizes[key])])
	print("    пулы полян: %s" % " · ".join(report))
